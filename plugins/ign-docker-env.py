...
focal = "ubuntu:focal"
IGN_VERSIONS = {"fortress": focal}


# TODO: migrate this to use a gpu python library
def get_gpu_params() -> List[str]:
    # check for nvidia
...
def build_rocker_command(
    igniton_release: str,
    linux_distro: str,
    docker_args: Optional[List[str]],
    vol_args: Optional[List[str]],
) -> List[str]:
...
def main() -> None:
    try:
        ...
        rocker_cmd = build_rocker_command(
            ignition_version, linux_distro, docker_args, vol_args
        )
        _check_call(rocker_cmd)
    except KeyError:
        logging.error(
            f"Invalid value given for IGN_RELEASE. Please choose from {list(IGN_VERSIONS.keys())}"
        )
    except KeyboardInterrupt:
        logging.info("docker-env was stopped with a Keyboard Interrupt.\n")
