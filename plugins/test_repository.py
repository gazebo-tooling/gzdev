...
class TestBase(unittest.TestCase):
    def setUp(self) -> None:
        self.config: Dict[str, Any] = repository.load_config_file(
            "config/_test_repository.yaml"
        )
...
class TestRepoKey(TestBase):
    def test_basic_ok(self) -> None:
        self.assertEqual(repository.get_repo_key("osrf", self.config), "ABC1234567890")
...
class TestProjectNameResolution(TestBase):
    def test_direct_match(self) -> None:
        project_config = repository.get_first_valid_project_config(
            "ignition-math6", self.config, "jammy"
        )
    ...
    def test_non_exist(self) -> None:
        self.assertIsNone(
            repository.get_first_valid_project_config("fooooo", self.config, "jammy")
        )

    def test_regexp(self) -> None:
        project_config = repository.get_first_valid_project_config(
            "ignition-plugin", self.config, "jammy"
        )
        self.assertIsNotNone(project_config)
        assert project_config is not None
        for p in repository.get_repositories_config(project_config):
    ...
    def test_precedence(self) -> None:
        project_config = repository.get_first_valid_project_config(
            "ignition-transport7", self.config, "jammy"
        )
        self.assertIsNotNone(project_config)
        assert project_config is not None
        for p in repository.get_repositories_config(project_config):
...
class TestProjectInstall(TestBase):
    def test_no_distributions(self) -> None:
        repository.process_project_install(
            "ignition-math6", self.config, "jammy", gpg_check=False, dry_run=True
        )

    def test_distributions(self) -> None:
        repository.process_project_install(
            "ignition-transport7", self.config, "jammy", gpg_check=False, dry_run=True
        )

    def test_non_exist(self) -> None:
        with self.assertRaises(SystemExit):
            repository.process_project_install(
                "fooooo", self.config, "jammy", gpg_check=False, dry_run=True
            )
